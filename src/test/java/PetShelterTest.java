...
public class PetShelterTest {
	...
	private Organic OPet;
	private Robotic RPet;
	...
	public void shouldAddOrganicDog() {
		...
		Organic OPet = new Organic(petName, petDescription);
		//act
		underTest.addPetOrganic(OPet);
		VirtualPet retrieved = underTest.findPet("TEST"); 
		
		//assert
		assertThat(retrieved, is(OPet)); 
	}

	@Test
	public void shouldAddRoboticDog() {
		underTest = new PetShelter(); 
		RoboDog RPet = new RoboDog(petName, petDescription); 
		underTest.addPetRobotic(RPet); 
		VirtualPet retrieved = underTest.findPet("TEST"); 
		assertThat(retrieved, is(RPet)); 
		}
	
	@Test 
	public void shouldAddOneRoboticCatAndOrganicCat() {
		underTest = new PetShelter();
		RoboCat RPet = new RoboCat(petName,petDescription); 
		OrganicCat OPet = new OrganicCat("TEST2","TESTING"); 
		underTest.addPetRobotic(RPet);
		underTest.addPetOrganic(OPet); 
		VirtualPet retrieved1 = underTest.findPet("TEST"); 
		VirtualPet retrieved2 = underTest.findPet("TEST2"); 
		assertThat(retrieved1, is(RPet)); 
		assertThat(retrieved2, is(OPet)); 
